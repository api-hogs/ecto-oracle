...
defmodule EctoOracleAdapterTest do
  ...
  test "from" do
    ...
    assert SQL.all(query) == ~s{SELECT m0."x" FROM "model" m0}
  end

  test "select" do
    query = Model |> select([r], {r.x, r.y}) |> normalize
    assert SQL.all(query) == ~s{SELECT m0."x", m0."y" FROM "model" m0}
  ...
  test "aggregates" do
    ...
    assert SQL.all(query) == ~s{SELECT count(m0."x") FROM "model" m0}

    query = Model |> select([r], count(r.x, :distinct)) |> normalize
    assert SQL.all(query) == ~s{SELECT count(DISTINCT m0."x") FROM "model" m0}
  end

  test "distinct" do
    query = Model |> distinct([r], r.x) |> select([r], {r.x, r.y}) |> normalize
    assert SQL.all(query) == ~s{SELECT DISTINCT ON (m0."x") m0."x", m0."y" FROM "model" m0}

    query = Model |> distinct([r], 2) |> select([r], r.x) |> normalize
    assert SQL.all(query) == ~s{SELECT DISTINCT ON (2) m0."x" FROM "model" m0}

    query = Model |> distinct([r], [r.x, r.y]) |> select([r], {r.x, r.y}) |> normalize
    assert SQL.all(query) == ~s{SELECT DISTINCT ON (m0."x", m0."y") m0."x", m0."y" FROM "model" m0}

    query = Model |> distinct([r], true) |> select([r], {r.x, r.y}) |> normalize
    assert SQL.all(query) == ~s{SELECT DISTINCT m0."x", m0."y" FROM "model" m0}
  ...
  test "where" do
    ...
    assert SQL.all(query) == ~s{SELECT m0."x" FROM "model" m0 WHERE (m0."x" = 42) AND (m0."y" != 43)}
  end

  test "order by" do
    query = Model |> order_by([r], r.x) |> select([r], r.x) |> normalize
    assert SQL.all(query) == ~s{SELECT m0."x" FROM "model" m0 ORDER BY m0."x"}

    query = Model |> order_by([r], [r.x, r.y]) |> select([r], r.x) |> normalize
    assert SQL.all(query) == ~s{SELECT m0."x" FROM "model" m0 ORDER BY m0."x", m0."y"}

    query = Model |> order_by([r], [asc: r.x, desc: r.y]) |> select([r], r.x) |> normalize
    assert SQL.all(query) == ~s{SELECT m0."x" FROM "model" m0 ORDER BY m0."x", m0."y" DESC}

    query = Model |> order_by([r], [r.y]) |> distinct([r], r.x) |> select([r], r.x) |> normalize
    assert SQL.all(query) == ~s{SELECT DISTINCT ON (m0."x") m0."x" FROM "model" m0 ORDER BY m0."x", m0."y"}
  ...
  test "limit and offset" do
    ...
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 LIMIT 3}

    query = Model |> offset([r], 5) |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 OFFSET 5}

    query = Model |> offset([r], 5) |> limit([r], 3) |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 LIMIT 3 OFFSET 5}
  end

  test "lock" do
    query = Model |> lock("FOR SHARE NOWAIT") |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 FOR SHARE NOWAIT}
  end

  test "string escape" do
    query = "model" |> where(foo: "'\\  ") |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM \"model\" m0 WHERE (m0.\"foo\" = '''\\  ')}
  ...
  test "binary ops" do
    ...
    assert SQL.all(query) == ~s{SELECT m0."x" = 2 FROM "model" m0}
  ...
  test "is_nil" do
    ...
    assert SQL.all(query) == ~s{SELECT m0."x" IS NULL FROM "model" m0}

    query = Model |> select([r], not is_nil(r.x)) |> normalize
    assert SQL.all(query) == ~s{SELECT NOT (m0."x" IS NULL) FROM "model" m0}
  end

  test "fragments" do
    query = Model |> select([r], fragment("now")) |> normalize
    assert SQL.all(query) == ~s{SELECT now FROM "model" m0}

    query = Model |> select([r], fragment("downcase(?)", r.x)) |> normalize
    assert SQL.all(query) == ~s{SELECT downcase(m0."x") FROM "model" m0}

    value = 13
    query = Model |> select([r], fragment("downcase(?, ?)", r.x, ^value)) |> normalize
    assert SQL.all(query) == ~s{SELECT downcase(m0."x", $1) FROM "model" m0}
  ...
  test "literals" do
    ...
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 WHERE (m0."foo" = TRUE)}

    query = "model" |> where(foo: false) |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 WHERE (m0."foo" = FALSE)}

    query = "model" |> where(foo: "abc") |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 WHERE (m0."foo" = 'abc')}

    query = "model" |> where(foo: <<0,?a,?b,?c>>) |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 WHERE (m0."foo" = '\\x00616263'::bytea)}

    query = "model" |> where(foo: 123) |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 WHERE (m0."foo" = 123)}

    query = "model" |> where(foo: 123.0) |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 WHERE (m0."foo" = 123.0::float)}
  end
  ...
  test "nested expressions" do
    ...
    assert SQL.all(query) == ~s{SELECT ((m0."x" > 0) AND (m0."y" > $1)) OR TRUE FROM "model" m0}
  end

  test "in expression" do
    query = Model |> select([e], 1 in []) |> normalize
    assert SQL.all(query) == ~s{SELECT false FROM "model" m0}

    query = Model |> select([e], 1 in [1,e.x,3]) |> normalize
    assert SQL.all(query) == ~s{SELECT 1 IN (1,m0."x",3) FROM "model" m0}

    query = Model |> select([e], 1 in ^[]) |> normalize
    assert SQL.all(query) == ~s{SELECT false FROM "model" m0}

    query = Model |> select([e], 1 in ^[1, 2, 3]) |> normalize
    assert SQL.all(query) == ~s{SELECT 1 IN ($1,$2,$3) FROM "model" m0}

    query = Model |> select([e], 1 in [1, ^2, 3]) |> normalize
    assert SQL.all(query) == ~s{SELECT 1 IN (1,$1,3) FROM "model" m0}

    query = Model |> select([e], 1 in e.w) |> normalize
    assert SQL.all(query) == ~s{SELECT 1 = ANY(m0."w") FROM "model" m0}

    query = Model |> select([e], 1 in fragment("foo")) |> normalize
    assert SQL.all(query) == ~s{SELECT 1 = ANY(foo) FROM "model" m0}
  end

  test "having" do
    query = Model |> having([p], p.x == p.x) |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 HAVING (m0."x" = m0."x")}

    query = Model |> having([p], p.x == p.x) |> having([p], p.y == p.y) |> select([], true) |> normalize
    assert SQL.all(query) == ~s{SELECT TRUE FROM "model" m0 HAVING (m0."x" = m0."x") AND (m0."y" = m0."y")}
  end

  test "group by" do
    query = Model |> group_by([r], r.x) |> select([r], r.x) |> normalize
    assert SQL.all(query) == ~s{SELECT m0."x" FROM "model" m0 GROUP BY m0."x"}

    query = Model |> group_by([r], 2) |> select([r], r.x) |> normalize
    assert SQL.all(query) == ~s{SELECT m0."x" FROM "model" m0 GROUP BY 2}

    query = Model |> group_by([r], [r.x, r.y]) |> select([r], r.x) |> normalize
    assert SQL.all(query) == ~s{SELECT m0."x" FROM "model" m0 GROUP BY m0."x", m0."y"}
  ...
  test "arrays and sigils" do
    ...
    assert SQL.all(query) == ~s{SELECT ARRAY[1,2,3] FROM "model" m0}

    query = Model |> select([], fragment("?", ~w(abc def))) |> normalize
    assert SQL.all(query) == ~s{SELECT ARRAY['abc','def'] FROM "model" m0}
  end
  ...
  test "interpolated values" do
    ...
    result =
      "SELECT $1 FROM \"model\" m0 INNER JOIN \"model2\" AS m1 ON $2 " <>
  ...
  test "join" do
    ...
    assert SQL.all(query) ==
           ~s{SELECT TRUE FROM "model" m0 INNER JOIN "model2" AS m1 ON m0."x" = m1."z"}
  ...
  test "join with nothing bound" do
    ...
    assert SQL.all(query) ==
           ~s{SELECT TRUE FROM "model" m0 INNER JOIN "model2" AS m1 ON m1."z" = m1."z"}
  end

  test "join without model" do
    query = "posts" |> join(:inner, [p], q in "comments", p.x == q.z) |> select([], true) |> normalize
    assert SQL.all(query) ==
           ~s{SELECT TRUE FROM "posts" p0 INNER JOIN "comments" AS c1 ON p0."x" = c1."z"}
  end

  test "join with prefix" do
    query = Model |> join(:inner, [p], q in Model2, p.x == q.z) |> select([], true) |> normalize
    assert SQL.all(%{query | prefix: "prefix"}) ==
           ~s{SELECT TRUE FROM "prefix"."model" m0 INNER JOIN "prefix"."model2" AS m1 ON m0."x" = m1."z"}
  end

  test "join with fragment" do
    query = Model
            |> join(:inner, [p], q in fragment("SELECT * FROM model2 m2 WHERE m2.id = ? AND m2.field = ?", p.x, ^10))
            |> select([p], {p.id, ^0})
            |> where([p], p.id > 0 and p.id < ^100)
            |> normalize
    assert SQL.all(query) ==
           ~s{SELECT m0."id", $1 FROM "model" m0 INNER JOIN } <>
           ~s{(SELECT * FROM model2 m2 WHERE m2.id = m0."x" AND m2.field = $2) AS f1 ON TRUE } <>
  ...
  test "association join belongs_to" do
    ...
    assert SQL.all(query) ==
           "SELECT TRUE FROM \"model2\" m0 INNER JOIN \"model\" AS m1 ON m1.\"x\" = m0.\"z\""
  end

  test "association join has_many" do
    query = Model |> join(:inner, [p], c in assoc(p, :comments)) |> select([], true) |> normalize
    assert SQL.all(query) ==
           "SELECT TRUE FROM \"model\" m0 INNER JOIN \"model2\" AS m1 ON m1.\"z\" = m0.\"x\""
  end

  test "association join has_one" do
    query = Model |> join(:inner, [p], pp in assoc(p, :permalink)) |> select([], true) |> normalize
    assert SQL.all(query) ==
           "SELECT TRUE FROM \"model\" m0 INNER JOIN \"model3\" AS m1 ON m1.\"id\" = m0.\"y\""
  end
  ...
  test "join produces correct bindings" do
    ...
    assert SQL.all(query) ==
           "SELECT m0.\"id\", m2.\"id\" FROM \"model\" m0 INNER JOIN \"model2\" AS m1 ON TRUE INNER JOIN \"model2\" AS m2 ON TRUE"
  end
